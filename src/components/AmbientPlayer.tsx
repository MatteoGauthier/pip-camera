...
const AmbientPlayer = ({ sources, interval = 5000, videoProps }: AmbientPlayerProps) => {
  ...
  const startExtractInterval = async () => {
    ...
    let [clientWidth, clientHeight] = [videoRef.current.clientWidth, videoRef.current.clientHeight]
    ...
    extractionInterval.current = setInterval(async () => {
      ...
      canvasRefs.current[nextIndex].context?.drawImage(videoRef.current, 0, 0, clientWidth, clientHeight)
  ...
  return (
    <div className="ambient-player-container">
      ...
      <video ref={videoRef} controls {...videoProps}>
        ...
        <p>Your browser doesn&rsquo;t support HTML video.</p>
      </video>
    </div>
  )
}
